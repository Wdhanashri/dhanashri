...
|---|---|
| 3 · Longest w/o repeats | no character appears twice |
| 424 · Char replacement | `window_len - max_freq <= k` |
| 567 · Permutation in string | window counts == pattern counts |
| 76 · Min window substring | window covers every needed char |

Write that sentence down *first*, in English. Then the code writes itself: grow while valid, shrink while not.

**The Gotcha:** in 424, `max_freq` doesn't need to be recomputed when the window shrinks. Leaving it stale is *correct* here — the answer can only be beaten by a genuinely larger `max_freq`. This looks like a bug and isn't.
...
**The Gotcha:** Min Stack (155) — store `(value, min_so_far)` as pairs, so `getMin()` is O(1). Recomputing the min on demand is the trap.
...
**The Gotcha:** order matters for `-` and `/`. The **second** value popped is the left operand: `a = stack.pop()` then `b = stack.pop()`, compute `b - a`. And Python's `//` floors toward negative infinity — LC 150 wants truncation toward zero, so use `int(b / a)`.
...
**Learn:** C-08.
**Exercise (10 min, paper):** trace L and R on `"abcabcbb"` for LC 3. Write the window contents and the best-so-far at every step. Do this *before* coding.

| # | Problem | Guidance | Cap |
|---|---|---|---|
| 121 | Best Time to Buy/Sell | Not really a window — just track `min_price` so far. Do it in 5 min as a warm-up. | 10m |
| 3 | Longest Substring w/o Repeat | The canonical window. Set or last-seen dict. If using a dict, `l = max(l, seen[c]+1)` — the `max` matters. | 25m |
...
| # | Problem | Guidance | Cap |
|---|---|---|---|
| 424 | Longest Repeating Char Replacement | Invariant: `window_len - max_freq <= k`. The stale-`max_freq` subtlety is the whole lesson. | 25m |
| 567 | Permutation in String | Fixed-size window. Slide it and compare counts — no shrinking loop needed. | 20m |

### D10 — Window + map
**Cold open:** ① C-09 anchor? ② The invariant for 424? ③ *(D8)* Why O(n)?
**Learn:** C-10.

| # | Problem | Guidance | Cap |
|---|---|---|---|
| 76 | Minimum Window Substring | The hardest window problem there is. Use the `matched` counter. Take the full 40 min — this one is worth it. | 40m |
...
| # | Problem | Guidance | Cap |
|---|---|---|---|
| 20 | Valid Parentheses | Map closers→openers. Check the stack is empty at the end — that's the commonly-missed case. | 10m |
| 155 | Min Stack | Store `(val, min_so_far)` pairs. O(1) `getMin`. | 15m |
| 22 | Generate Parentheses | ⚠️ This is **backtracking**, not a stack — NeetCode files it here anyway. Rule: add `(` if `open < n`, add `)` if `close < open`. A preview of Week 6. | 25m |
...
| # | Problem | Guidance | Cap |
|---|---|---|---|
| 739 | Daily Temperatures | The textbook monotonic stack. Store indices. | 20m |
| 853 | Car Fleet | Sort by position descending, then compute arrival times. A fleet forms when a car's time ≤ the stack top. | 25m |
| 239 | Sliding Window Maximum | Monotonic **deque** — same idea, both ends. Pop from the back while smaller; pop from the front when out of window. | 30m |
...
| # | Problem | Guidance | Cap |
|---|---|---|---|
| 150 | Evaluate RPN | Watch the operand order and the `int(b/a)` truncation. | 15m |
| 84 | Largest Rectangle in Histogram | The hardest problem in Tier 1. Monotonic increasing stack of `(index, height)`. When you pop, that bar's rectangle extends back to the popped index. **If you're stuck at 30 min, read the solution and log it — this one legitimately takes two attempts.** | 40m |
...
**Part B — Cold problems (45 min).** From an empty file: **3**, **739**, **76**. Pass = 2/3 working.
...
**Part E — Interleaved set (20 min).** Look at these five, **name the pattern only — don't solve**: LC 424 · LC 128 · LC 853 · LC 15 · LC 347. Pattern recognition is the skill NeetCode's section headings quietly do for you. Here, nobody does.
